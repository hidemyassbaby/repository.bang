...
class Source(database.Model):
    ...
    def select_path(self, creating=False):
        ...
        if self.source_type == self.TYPE_ADDON:
            ...
            self.path = gui.input(_.ENTER_SOURCE_URL, default=self.path if orig_source_type == self.TYPE_URL else '')
            ...
            self.path = gui.input('Custom Name', default=self.path if orig_source_type == self.TYPE_CUSTOM else '')
    ...
    def select_archive_type(self):
        values = [self.ARCHIVE_NONE, self.ARCHIVE_GZIP]
        labels = [_.NOT_ARCHIVED, _.GZIP]
...
class Channel(database.Model):
    ...
    @classmethod
    def playlist_list(cls, radio=False):
        query = cls.select(cls).join(Playlist).where(cls.visible == True, cls.radio==radio).order_by(cls.chno.asc(nulls='LAST'), cls.playlist.order, cls.order)
